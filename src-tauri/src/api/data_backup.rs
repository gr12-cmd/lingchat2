...
#[tauri::command]
pub fn peek_data_backup(src_path: String) -> Result<BackupManifest, String> {
...
pub async fn import_data_backup(
    ...
    let dst = temp_dir.clone();
    tokio::task::spawn_blocking(move || extract_zip_to_dir_sync(&src, &dst))
        .await
...
fn create_zip_from_dir(dir: &Path, zip_path: &Path) -> Result<(), String> {
    ...
    for path in &files {
        ...
        f.read_to_end(&mut buf)
            .map_err(|e| format!("读取文件 {}: {e}", path.display()))?;
        zip.write_all(&buf)
            .map_err(|e| format!("写入 zip: {e}"))?;
    }
